  ...
  return (
    <div>
      <div className="flex flex-col gap-2.5 mb-5">
        {skillAreas.map((sa) => (
          <form key={sa.id} action={renameSkillArea} className="flex items-center gap-2.5">
            ...
            <button
              type="submit"
              className="bg-[var(--color-neutral)] border-none text-[var(--color-orange-dark)] transition-colors hover:bg-[var(--color-neutral-divider)] font-bold text-xs px-3 py-1.5 rounded-full cursor-pointer whitespace-nowrap"
            >
              Save
            </button>
            {sa.inUse ? (
      ...
      <form action={addSkillArea} className="flex flex-wrap items-end gap-2.5 border-t border-[var(--color-neutral-divider)] pt-4">
        ...
        <button
          type="submit"
          className="bg-[var(--color-orange)] text-white border-none rounded-full font-bold text-sm shadow-[0_6px_16px_rgba(74,107,82,0.25)] transition-transform hover:-translate-y-0.5 px-4.5 py-2 cursor-pointer"
        >
          Add skill area
        </button>
      </form>
